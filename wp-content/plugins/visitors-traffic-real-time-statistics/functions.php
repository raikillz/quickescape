...
function ahcfree_google_map($map_option) {
    ...
    if ($map_option == 'online') {
        $ctrArr = ahcfree_get_online_visitors_for_map();
    } else if (is_array($map_option) and $map_option['map_status'] == 'online') {
        $ctrArr = ahcfree_get_online_visitors_for_map();
    } else if (is_array($map_option) and $map_option['map_status'] != 'online') {
        $ctrArr = ahcfree_get_today_visitors_for_map();
    } else {
        $ctrArr = ahcfree_get_today_visitors_for_map();
    }
    $ret = '<div id="googleMap" style="width: 100%; height:550px;"></div>
<script language="javascript" type="text/javascript">
		jQuery(document).ready(function(){
				function initializemap(){
				var mapProp = {
				center: new google.maps.LatLng(34.307144,-8.789062),
				zoom: 2,
    ...
    if ($ctrArr['success'] && isset($ctrArr['data']) && count($ctrArr['data']) > 0) {
        foreach ($ctrArr['data'] as $vc) {
            ...
            $info .= '</div>';
            $ret .= 'var LatLng' . $counter . ' = new google.maps.LatLng(' . $vc['ctr_latitude'] . ',' . $vc['ctr_longitude'] . ');
			...
			marker' . $counter . '.setMap(map);
			
			google.maps.event.addListener(marker' . $counter . ', \'mouseover\', function(){
				infowindow' . $counter . '.open(map, marker' . $counter . ');
			});
			
			google.maps.event.addListener(marker' . $counter . ', \'mouseout\', function(){
				infowindow' . $counter . '.close();
			});';
            $counter++;
        }
    }
...
function ahcfree_savesettings() {
    ...
    $set_hits_days = $_POST['set_hits_days'];
    $set_ajax_check = 15;
    $posts_type = '';
    $set_ips = $_POST['set_ips'];
    $set_google_map = '';
...
function ahcfree_create_admin_menu_link() {
    add_menu_page('Visitors Traffic Real Time Statistics', 'Visitors Traffic', 'manage_options', 'ahc_hits_counter_menu_free', 'ahcfree_create_plugin_overview_page', plugins_url('/images/vtrts.png', AHCFREE_PLUGIN_MAIN_FILE));
    add_submenu_page('ahc_hits_counter_menu_free', 'Visitors Traffic Real Time Statistics Settings', 'Settings', 'manage_options', 'ahc_hits_counter_settings', 'ahcfree_create_plugin_settings_page');
...
function ahcfree_countOnlineusers() {
    ...
    /*$sql = "SELECT count(hits.visitors) as onlineusers FROM (
				SELECT COUNT(v.visitor) AS visitors, v.ctr_id FROM (
				SELECT ctr_id, 1 AS visitor FROM `ahc_hits`
				WHERE ctr_id IS NOT NULL and hit_date = DATE( CONVERT_TZ(NOW( ),'" . AHCFREE_SERVER_CURRENT_TIMEZONE . "','" . $custom_timezone_offset . "') ) and TIME( CONVERT_TZ(hit_time,'" . AHCFREE_SERVER_CURRENT_TIMEZONE . "','" . $custom_timezone_offset . "') ) between TIME(CONVERT_TZ(now(),'" . AHCFREE_SERVER_CURRENT_TIMEZONE . "','" . $custom_timezone_offset . "') - INTERVAL 60 SECOND) and TIME(CONVERT_TZ( NOW( ),'" . AHCFREE_SERVER_CURRENT_TIMEZONE . "','" . $custom_timezone_offset . "') ) AND hit_ip_address NOT LIKE 'UNKNOWN%'
    ...
    $sql = "SELECT count(hits.visitors) as onlineusers FROM (
			SELECT COUNT(v.visitor) AS visitors, v.ctr_id FROM (
			SELECT ctr_id, 1 AS visitor FROM `ahc_hits`
			WHERE ctr_id IS NOT NULL and hit_date = DATE( NOW( ) ) and TIME( hit_time ) between time(now() - INTERVAL 60 SECOND) and TIME( NOW( ) ) AND hit_ip_address NOT LIKE 'UNKNOWN%'
			GROUP BY hit_ip_address 
			) AS v 
			GROUP BY ctr_id) AS hits 
			JOIN `ahc_countries` AS c ON hits.ctr_id = c.ctr_id 
...
function ahcfree_get_serch_visits_by_date() {
    global $wpdb;
    $searchEngines = ahcfree_get_all_search_engines();
    ...
    $sql = "SELECT srh_id, vtsh_visits, DATE(CONVERT_TZ(vtsh_date,'" . AHCFREE_SERVER_CURRENT_TIMEZONE . "','" . $custom_timezone_offset . "')) as vtsh_date 
			FROM `ahc_searching_visits` 
			WHERE DATE(CONVERT_TZ(vtsh_date, '" . AHCFREE_SERVER_CURRENT_TIMEZONE . "', '" . $custom_timezone_offset . "')) >= DATE(CONVERT_TZ(%s, '" . AHCFREE_SERVER_CURRENT_TIMEZONE . "', '" . $custom_timezone_offset . "'))";
...
/*function ahcfree_track_visitor($query) {
    ...
    if (ahcfree_should_track_visitor() && !ahcfree_is_login_page() && !ahcfree_is_search_engine_bot() && !ahcfree_is_wordpress_bot()) {
        if (!in_array(ahcfree_get_client_ip_address(), $exclude_ips)) {
            ...
            if (is_array($page)) {
                Globalsahcfree::$page_id = $page['page_id'];
                Globalsahcfree::$page_title = $page['page_title'];
                Globalsahcfree::$post_type = $page['post_type'];
            } else {
                return;
            }
            $hitsCounter = new AHCFree_WPHitsCounterPro(Globalsahcfree::$page_id, Globalsahcfree::$page_title, Globalsahcfree::$post_type);
...
function ahcfree_include_scripts() {
    wp_register_style('ahcfree_lang_css', plugins_url('/css/en_css.css', AHCFREE_PLUGIN_MAIN_FILE));
    ...
    wp_register_script('ahcfree_main_js', plugins_url('/js/javascript.js', AHCFREE_PLUGIN_MAIN_FILE));
    ...
    //wp_register_script('ahcfree_google_maps', 'https://maps.googleapis.com/maps/api/js?key=AIzaSyB0fRgC_3Wmp1PY5ZsuzK8VEooiUvVQq3Q&sensor=false');
    //wp_enqueue_script('ahcfree_google_maps');
	...
	wp_enqueue_style( 'sweetalert', plugins_url('/css/sweetalert.css', AHCFREE_PLUGIN_MAIN_FILE));
...
//The properties of the new item. Read More about the missing 'parent' parameter below
    $args = array(
        'id' => 'visitorstraffic',
        'title' => __('<img src="' . plugins_url('/images/vtrtspro.png', AHCFREE_PLUGIN_MAIN_FILE) . '" style="vertical-align:middle;margin-right:5px;" alt="visitors traffic" title="visitors traffic" />'),
...
function ahcfree_vtrtsp_plugin_add_settings_link($links) {
    $settings_link = '<a href="admin.php?page=ahc_hits_counter_menu_pro">' . __('visitors traffic') . '</a>';
...
// Widget name will appear in UI
                __('Visitors Traffic', 'wpb_widget_domain'),
// Widget description
...
function ahcfree_admin_notice_to_set_timezone(){
	...
	$name = 'Visitors Traffic Real Time Statistics free';
